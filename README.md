# 🧑‍💻 Higor Pereira dos Santos - Desenvolvedor e Estudante de Ciência da Computação

Olá! Meu nome é Higor e sou estudante de Ciência da Computação na Universidade Paulista (UNIP). Tenho interesse em desenvolvimento de software, criação de interfaces desktop e integração com APIs, além de estar explorando Inteligência Artificial, Visão Computacional e análise de dados conforme avanço nos meus estudos.

Gosto de transformar ideias em soluções práticas — desde ferramentas para o dia a dia e aplicações desktop em Java até projetos experimentais em Python envolvendo automação, organização de dados e técnicas iniciais de IA. No meu GitHub, mantenho projetos que mostram essa evolução, passando por interfaces gráficas, consumo de APIs, criptografia e análises aplicadas.


## 💻 Habilidades Técnicas

- **Linguagens:** Java, Python, JavaScript  
- **Front-End:** HTML, CSS, Bootstrap  
- **Aplicações Desktop:** JavaFX, Swing  
- **Banco de Dados:** MySQL, SQL Server  
- **APIs e Integrações:** Consumo de APIs REST e manipulação de dados externos  
- **Ferramentas:** Git, Maven, Eclipse  
- **IA e Dados:** Visão Computacional e Processamento Digital de Imagens (nível iniciante)

## 📊 Minhas Habilidades
![Habilidades](https://skillicons.dev/icons?i=java,python,mysql,javascript,css,html,bootstrap,github&theme=light)

## 📫 Contato

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/higor-pereira-761081200/)
[![Gmail](https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:higor.pereira368@gmail.com)
